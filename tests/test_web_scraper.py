...
def test_get_citations_needed_count(fixed_site):
    actual = get_citations_needed_count(fixed_site)
    expected = 'The Number Of Citations Needed 5'
    assert actual == expected


def test_get_citations_needed_report(fixed_site):
    actual = get_citations_needed_report(fixed_site)
...
@pytest.fixture
def fixed_site():
    return 'https://en.wikipedia.org/wiki/History_of_Mexico'
